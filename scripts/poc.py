...
async def main():
    ...
    async with Toggl(_key) as api:
        ...
        log.info("account details are:", account=me)

        log.info("Fetching workspaces")
        w = await api.get_workspaces()
        log.info("Got workspace!", count=len(w), workspaces=w)
        ...
        workspace_id = w[0].id
        log.info("Using workspace", workspace_id=workspace_id)
        ...
        if te is None:
            ...
            log.info("get_current_time_entry", entry=te)
        ...
        log.info("stop_time_entry", result=result)
        ...
        log.info("get_time_entries", count=len(te), entries=te)
